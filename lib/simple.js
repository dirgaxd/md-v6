...
conn.sendMessageModify = async (jid, text, quoted, properties, options = {}) => {
    ...
    */
    ...
    conn.send5TemplateButtonImg = async (jid, buffer, content, footerText, button1, id1, button2, id2, button3, id3, button4, id4, button5, id5, quoted, options) => {
        let type = await conn.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 200000) {
        try { throw { json: JSON.parse(file.toString()) } }
  ...
  conn.send5TemplateButtonLoc = async (jid, buffer, contentText, footer, buttons1, row1, buttons2, row2, buttons3, row3, buttons4, row4, buttons5, row5, quoted, options) => {
    const type = await conn.getFile(buffer)
        let { res, data: file } = type
        if (res && res.status !== 200 || file.length <= 200000) {
        try { throw { json: JSON.parse(file.toString()) } }
