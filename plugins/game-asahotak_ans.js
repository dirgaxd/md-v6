...
handler.before = async function (m) {
    ...
    if (!(id in this.asahotak)) return m.reply('Soal asahotak itu telah berakhir')
    if (m.quoted.id == this.asahotak[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            await this.sendBut(m.chat, `*Benar!* +${this.asahotak[id][2]} XP`, 'By K.1', 'Asah Otak', '.asahotak', m)
