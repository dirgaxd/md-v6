...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (id in conn.tebakkata) {
        conn.reply(m.chat, 'Masih ada soal tebakkata belum terjawab di chat ini', conn.tebakkata[id][0])
        throw false
    }
...
`.trim()
    conn.tebakkata[id] = [
        ...
        json, poin,
        setTimeout(async () => {
            if (conn.tebakkata[id]) await conn.sendBut(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, '© Kimimaru Md', 'Tebak Kata', '.tebakkata', conn.tebakkata[id][0])
